<template>
	...
	<FormKit type="form" @submit="create" :incomplete-message="false" :actions="false">
		<div class="row gy-4">
			<div class="col-md-3">
				<FormKit type="text" id="code" name="code" label="کد تخفیف" label-class="form-label"
					input-class="form-control" messages-class="form-text text-danger" validation="required"
					:validation-messages="{ required: 'فیلد کد اجباریست' }" />
			</div>
			<div class="col-md-3">
				<FormKit type="text" id="percentage" name="percentage" label="درصد" label-class="form-label"
					input-class="form-control" messages-class="form-text text-danger" validation="required|number"
					:validation-messages="{ required: 'فیلد درصد اجباریست', number: 'باید عدد باشد' }" />
			</div>
			<ClientOnly>
				<div class="col-md-3">
			...
			<FormKit :disabled="loading" type="submit" input-class="btn btn-outline-dark mt-3">
				ایجاد تخفیف
				<div v-if="loading" class="spinner-border-sm spinner-border-sm ms-2"></div>
			</FormKit>
		</div>
	</FormKit>
</template>
...
async function create(formData) {
	if (!dateExpire.value) {
		toast.error('تاریخ انقضا الزامیست')
		return;
	}
	try {
		loading.value = true;
