<template>
	...
	<ClientOnly fallback-tag="span" fallback="در حال بارگذاری...">
		<FormKit type="form" @submit="edit" :incomplete-message="false" :actions="false">
			<div class="row">
				<ProductPrimaryImage :primaryImage="product.primary_image"
					@setPrimaryImage="(image) => (primaryImage = image)" />
				<div class="col-md-3 mt-5">
					<FormKit :value="product.name" type="text" name="name" id="name" label="نام" label-class="form-label"
				...
				<div class="col-md-3 mt-5">
					<FormKit :value="product.category_id" type="select" name="category_id" id="category_id"
						label="دسته بندی" label-class="form-label" input-class="form-select"
				...
				<div class="col-md-3 mt-5">
					<FormKit :value="product.status_value" type="select" name="status" id="status" label="وضعیت"
						label-class="form-label" input-class="form-select" messages-class="form-text text-danger"
						validation="required" :validation-messages="{ required: 'فیلد وضعیت اجباریست' }">
						<option value="1">فعال</option>
						<option value="0">غیرفعال</option>
					</FormKit>
				</div>
				<div class="col-md-3 mt-5">
					<FormKit :value="product.price" type="text" name="price" id="price" label="قیمت"
				...
				<div class="col-md-3 mt-4">
					<FormKit :value="product.quantity" type="text" name="quantity" id="quantity" label="تعداد"
						label-class="form-label" input-class="form-control" messages-class="form-text text-danger"
						validation="required|number" :validation-messages="{
				...
				<div class="col-md-3 mt-4">
					<FormKit :value="product.sale_price" type="text" name="sale_price" id="sale_price" label="قیمت حراجی"
						label-class="form-label" input-class="form-control" messages-class="form-text text-danger"
				...
				<div class="col-md-3 mt-4">
					<label for="start-sale-date" class="form-label">تاریخ شروع حراجی</label>
					<div class="input-group mb-3">
						<span class="input-group-text"> <i class="bi bi-clock"></i></span>
						<input :placeholder="product.date_on_sale_from_jalali" type="text" class="form-control"
							id="start-sale-date" />
					</div>
					<date-picker v-model="saleDateFrom" custom-input="#start-sale-date" type="datetime"
						format="YYYY-MM-DD HH:mm:ss" display-format="HH:mm jYYYY-jMM-jDD" />
				</div>
				<div class="col-md-3 mt-4">
					<label for="start-sale-date" class="form-label">تاریخ پایان حراجی</label>
					<div class="input-group mb-3">
						<span class="input-group-text"> <i class="bi bi-clock"></i></span>
						<input :placeholder="product.date_on_sale_to_jalali" type="text" class="form-control"
							id="end-sale-date" />
					</div>
					<date-picker v-model="saleDateEnd" custom-input="#end-sale-date" type="datetime"
						format="YYYY-MM-DD HH:mm:ss" display-format="HH:mm jYYYY-jMM-jDD" />
				</div>
				<div class="col-md-12 mt-5">
					<FormKit :value="product.description" type="textarea" name="description" rows="5" id="description"
						label="توضیحات" label-class="form-label" input-class="form-control"
						messages-class="form-text text-danger" validation="required"
						:validation-messages="{ required: 'فیلد توضیحات اجباریست' }" />
				</div>
				<div class="col-md-5 mt-4">
					<label for="images" class="form-label">تصاویر</label>
					<input @change="setImages" type="file" id="images" class="form-control" multiple />
				</div>
				<div class="col-md-12 text-center">
					<img v-for="image in product.images" :key="image.id" src="/images/preloader.png" width="300"
						class="mx-3 mt-5" v-img="image.image" />
				</div>
				<FormKit :disabled="loading" type="submit" input-class="btn btn-outline-dark my-5">
					ایجاد محصول
...
async function edit(data) {
	...
	try {
		...
		await $fetch(`${apiBase}/products/${product.value.id}`, {
			method: "POST",
			body: formData,
			headers: {
